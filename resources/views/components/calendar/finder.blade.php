		...
		<div class="px-4 py-3 bg-light">
			<form method="GET" action="/procurar">
				...
				<div class="row w-100 mx-auto">
					...
					<select name="time" class="col-lg-2 col-4 form-control rounded-0 border-left-0">
						@for($i=8; $i<=18; $i++)
						<option value="{{$i}}">{{$i}}:00h</option>
						@endfor
					</select>
					
					<select name="duration" class="col-lg-2 col-4 form-control rounded-0 border-left-0">
					...
					<select id="select-participants-co-working" name="participants" class="participants col-lg-2 col-4 form-control rounded-0 border-left-0 border-right-0">
						@for($i=1; $i<=12; $i++)
						<option value="{{$i}}">{{$i .' '. trans_choice('words.pessoas', $i)}}</option>
						@endfor
					</select>

					<select style="display: none;" id="select-participants-conference" class="participants col-lg-2 col-4 form-control rounded-0 border-0 border-y">
						@for($i=1; $i<=6; $i++)
						<option value="{{$i}}">{{$i .' '. trans_choice('words.pessoas', $i)}}</option>
						@endfor
					</select>
