<div class="col-lg-8 col-10 mx-auto">
    <form method="POST" action="{{ route('register') }}" class="mb-3">
        ...
        <div class="form-group">
            <input required type="text" name="name" class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}" aria-describedby="name" placeholder="Nome completo" value="{{old('name')}}">

            @include('components/form/error', ['field' => 'name'])
        </div>
        <div class="form-group">
            <input required type="text" name="phone" class="form-control {{ $errors->has('phone') ? ' is-invalid' : '' }}" aria-describedby="phone" placeholder="Telefone" value="{{old('phone')}}">

            @include('components/form/error', ['field' => 'phone'])
        </div>
        <div class="form-group">
            <input required type="email" name="email" class="form-control {{ $errors->has('email') ? ' is-invalid' : '' }}" aria-describedby="email" placeholder="Email" value="{{old('email')}}">
        ...
        <div class="form-group">
            <input required type="password" name="password_confirmation" class="form-control {{ $errors->has('password-confirm') ? ' is-invalid' : '' }}" aria-describedby="password-confirm" placeholder="Confirme o seu password">
        </div>
