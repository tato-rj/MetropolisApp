...
class EventTest extends AppTest
{
	...
	{
		create(Bonus::class, ['event_id' => $this->event]);
		$this->assertInstanceOf(Bonus::class, $this->event->bonus);
	}
	...
	public function it_checks_and_marks_any_conflicts_upon_creation_on_a_non_shared_space()
	{
		...
		$event1 = create(Event::class, ['space_id' => $nonSharedSpace->id, 'participants' => 1]);
	
		$event2 = create(Event::class, ['space_id' => $nonSharedSpace->id, 'participants' => 1]);
